...
|-------------|--------------------------------------------------------------------|
| Version     | 1.0.0                                                              |
| Status      | ACTIVE — PLAN (awaiting VALIDATE checkpoint from project owner)   |
| Date        | 2026-07-04                                                         |
...
Stillwater is an **enterprise-grade yoga studio management platform** — a Turborepo monorepo that combines a public marketing surface (Next.js 16 + Sanity CMS, ISR), a member application (booking, dashboard, membership, real-time seat availability via SSE), an admin surface (RBAC-gated class/schedule/member/revenue management), a background-job infrastructure (Trigger.dev v3), and Stripe subscription billing.
...
9. **Idempotency for webhooks** — Stripe webhook events deduplicated via `payment_events.stripe_event_id` UNIQUE INDEX + `pg_advisory_lock`.
...
- **Purpose:** Stripe webhook event handlers. Idempotent via `payment_events` table + `pg_advisory_lock`.
- **Interface:**
  ```typescript
...
**Goal:** All 11 background jobs from PAD §13.1 implemented as Trigger.dev v3 tasks. Jobs are durable, retried, and observable in Trigger.dev dashboard.
...
| R3 | SSE on Vercel Serverless may hit 25s function timeout               | Medium     | High   | Phase 5 uses `runtime = 'nodejs'` + `dynamic = 'force-dynamic'`; considers Pusher fallback if timeout fires |
...
| R5 | Stripe webhook duplicate processing                                | Low        | High   | `payment_events.stripe_event_id` UNIQUE INDEX + `pg_advisory_lock` (D12 + ADR-004)            |
